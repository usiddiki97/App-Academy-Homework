...
describe Dessert do
  let(:chef) { double("chef", name: "Umarbin") }
  let(:cookie) {Dessert.new("cookie", 20, chef) }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("cookie", "lol", chef) }.to  raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ["flour", "eggs", "baking soda", "vanilla extract", "sugar", "chocolate chips"]
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Umarbin the Great Baker")
      expect(cookie.serve).to eq("Chef Umarbin the Great Baker has made 20 cookies!")
    end
  end
